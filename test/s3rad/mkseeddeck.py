...
def mesh(centres, smin, smax, dmax, sin, algo3d, verbose):
    import gmsh
    gmsh.initialize()
...
def main():
    ...
    header.append('Mesh: smin %.4g smax %.4g dmax %.4g sin %.4g; all cards after '
                  'the sets copied from m12_s3rad_gc24_w.inp sha256 %s'
                  % (a.smin, a.smax, a.dmax, a.sin, SRC_SHA[:16]))
