...
#define TD_NMODE 3
	...
	if(*mortar<=1){
	  ...
	  /* ---- [DAMAGE TR] everything the dogleg needs from J, taken while
	     J is still allocated: dogleg_capture() in dogleg.c, called here
	     because the two lines below free ad and au and every trial
	     residual evaluation happens after that.  It proves the transpose
	     rather than asserting it, and REFUSES TO ARM when the identity
	     dot(J^T R,p_N)/|R|^2 is off by more than 1e-3. */
	  if(((dog.on==1)||(dog.lc_due==1))&&
	     (dog.r0!=NULL)&&(*mortar<=1)){
	    dogleg_capture(&dog,&prb,&nlgt,&nls,ad,au,symmetryflag);
	  }
	  SFREE(ad);SFREE(au);
	} 
      }
    ...
    if((dog.lc_due==1)&&(dog.have==1)){
      dogleg_lincheck(&dog,&prb,&nlgt,&nls,damage_addiag,damage_addiag0,
                      damage_damjac,damage_damvisc,damdamageini);
    }
